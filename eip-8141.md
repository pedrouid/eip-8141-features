...
Scheduled activation (a tx that becomes valid only after some future time) is intentionally out of scope. Any wallet or service holding a signed tx can solve it offchain by deferring submission, and a corresponding future-valid envelope field would impose buffering, gossip-threshold rules, per-sender caps, and reverse-window resolution on every node, with the envelope cost paid by every tx that does not use it. The field is named `expiry` to match the vocabulary every wallet, contract, and offchain protocol already uses for the concept.
