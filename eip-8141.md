...
### Expiry is envelope-level and one-sided
...
Only the upper bound (deadline) is included. The dominant use-case is intent-style flows (signed-order DEXes, cross-chain bridges, RFQ aggregators, gasless swaps): every such system today carries a deadline inside an offchain message that a relayer or filler is supposed to honor, with the protocol having no view of it. A consensus-enforced `expiry` collapses that timing layer into the tx itself, so no filler can stall a signed intent past its deadline and still land it on-chain. The same field also covers swap routers, liquidations, atomic-swap deadlines, async user actions, and MEV-sensitive submission.

A lower bound (`valid_after`) was considered and dropped: it would have doubled the envelope cost paid by every tx, added a future-valid mempool state that requires local buffering, a gossip threshold, per-sender caps, and reverse-window resolution, all for a minority use-case (scheduled activation) that any wallet or service holding a signed tx can solve offchain by deferring submission. The field is named `expiry` to match the vocabulary every wallet, contract, and offchain protocol already uses for the concept.
