...
The two halves do not earn that weight equally. `valid_before` (deadlines) is the dominant use-case: every intent system, swap router, trading flow, liquidation, atomic-swap protocol, and async user-action pattern uses one, and protocol enforcement collapses filler honesty about timing into a consensus rule. `valid_after` (scheduled activation) is a minority use-case solvable offchain: a wallet or service holding a signed tx can delay submission until the activation time. The "ship a tx now that nodes hold for an hour" pattern saves the signer one round-trip but costs every node mempool buffer space, a deferral horizon, a gossip-threshold rule, and an envelope field every other tx pays for.
