...
**Envelope expiry**
While it's a completely separate with a tiny surface: one `uint64` field, no system contract, no per-account state. It provides the offchain deadline used by every intent, swap, and settlement flow into a consensus-enforced field, removing filler timing as a trust assumption. Zero per-tx state beyond the envelope itself; it composes cleanly with the statelessness roadmap.
